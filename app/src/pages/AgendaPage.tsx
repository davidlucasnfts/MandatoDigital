...
export default function AgendaPage() {
  ...
  // Check if an event exists for a day
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-7 gap-1">
                {diasSemana.map(d => (
                  <div key={d} className="text-center text-xs font-semibold text-slate-400 uppercase py-2">{d}</div>
                ))}
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  const isToday = day === 23; // simulate today
                  return (
                    <div
                      key={day}
                      className={`aspect-square p-1.5 rounded-lg border transition-colors cursor-pointer hover:bg-slate-50 ${
                        isToday ? 'border-blue-300 bg-blue-50' : 'border-transparent'
                      }`}
                    >
                      <div className={`text-sm font-medium ${isToday ? 'text-blue-600' : 'text-slate-700'}`}>{day}</div>
                      {dayEvents.length > 0 && (
                        <div className="mt-0.5 space-y-0.5">
                          {dayEvents.slice(0, 2).map((e, j) => (
                            <div key={j} className="text-[9px] bg-blue-100 text-blue-700 px-1 py-0.5 rounded truncate leading-tight">
                              {e.titulo}
                            </div>
                          ))}
                          {dayEvents.length > 2 && (
        ...
        <motion.div custom={2} variants={fadeIn} initial="hidden" animate="visible">
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              {eventos.map((e) => (
                <div key={e.id} className="p-3 bg-slate-50 rounded-lg hover:bg-blue-50 transition-colors cursor-pointer">
                  <div className="flex items-start gap-2">
                    <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                      <Calendar className="w-4 h-4 text-blue-600" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-sm font-medium text-slate-800 truncate">{e.titulo}</h4>
                      ...
                      <div className="text-xs text-slate-400 mt-1">{e.data}</div>
                    </div>
                  </div>
                </div>
              ))}
