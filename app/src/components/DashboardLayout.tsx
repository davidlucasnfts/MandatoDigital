...
export default function DashboardLayout({ user, logout }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:static top-0 left-0 z-50 h-full bg-white border-r border-slate-200 flex flex-col transition-all duration-300 ${
      ...
      >
        ...
        <div className="p-3 border-t border-slate-100">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-3 px-3 py-2 w-full text-sm font-medium text-red-500 hover:bg-red-50 rounded-lg transition-colors mt-1"
          >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80">
                ...
                <div className="p-3 text-sm text-slate-500">
                  Nova solicitação de Maria Aparecida Santos
                </div>
                <div className="p-3 text-sm text-slate-500 border-t">
                  Tarefa "Visita ao posto de saúde" vence amanhã
                </div>
                <div className="p-3 text-sm text-slate-500 border-t">
                  E-mail de aniversário enviado para 12 contatos
                </div>
              </DropdownMenuContent>
            </DropdownMenu>
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 hover:bg-slate-100 rounded-lg px-2 py-1 transition-colors">
                  <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 font-semibold text-sm">{user?.nome?.charAt(0) || 'U'}</span>
                  </div>
                  <span className="hidden lg:block text-sm font-medium text-slate-700">{user?.nome || 'Usuário'}</span>
                </button>
              </DropdownMenuTrigger>
