...
import { Users, Search, Plus, Download, Upload, CheckCircle, XCircle, MessageSquare, ChevronDown } from 'lucide-react';
...
export default function EleitoresPageV2() {
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 lg:gap-4">
        <motion.div custom={1} variants={fadeIn} initial="hidden" animate="visible">
          <div className="rounded-xl border border-slate-200 bg-white border-t-[3px] border-t-blue-600 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 p-3 lg:p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg bg-blue-50 flex items-center justify-center">
                <Users className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-blue-600" />
              </div>
            </div>
            <div className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">{eleitores.length}</div>
            <div className="text-[10px] lg:text-xs text-slate-500 font-medium mt-1">Total de eleitores</div>
          </div>
        </motion.div>
        <motion.div custom={2} variants={fadeIn} initial="hidden" animate="visible">
          <div className="rounded-xl border border-slate-200 bg-white border-t-[3px] border-t-green-600 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 p-3 lg:p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg bg-green-50 flex items-center justify-center">
                <CheckCircle className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-green-600" />
              </div>
            </div>
            <div className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">{totalAtivos}</div>
            <div className="text-[10px] lg:text-xs text-slate-500 font-medium mt-1">Ativos</div>
          </div>
        </motion.div>
        <motion.div custom={3} variants={fadeIn} initial="hidden" animate="visible">
          <div className="rounded-xl border border-slate-200 bg-white border-t-[3px] border-t-purple-600 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 p-3 lg:p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg bg-purple-50 flex items-center justify-center">
                <Users className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-purple-600" />
              </div>
            </div>
            <div className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">{totalLideres}</div>
            <div className="text-[10px] lg:text-xs text-slate-500 font-medium mt-1">Líderes</div>
          </div>
        </motion.div>
        <motion.div custom={4} variants={fadeIn} initial="hidden" animate="visible">
          <div className="rounded-xl border border-slate-200 bg-white border-t-[3px] border-t-amber-600 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 p-3 lg:p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg bg-amber-50 flex items-center justify-center">
                <ChevronDown className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-amber-600" />
              </div>
            </div>
            <div className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight">{pendentesCount}</div>
            <div className="text-[10px] lg:text-xs text-slate-500 font-medium mt-1">Pendentes</div>
          </div>
        </motion.div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {(abaAtiva === 'pendentes' ? ['Ações','Nome','Contato','CPF','Comunidade','Líder','Nível','Tags','Status','Interações'] : ['Nome','Contato','CPF','Comunidade','Líder','Nível','Tags','Status','Interações']).map(h => (
                ...
                {loading ? (
                  Array.from({length:5}).map((_,i) => (
                    <tr key={i} className="border-b border-slate-50">
                      <td colSpan={abaAtiva === 'pendentes' ? 10 : 9} className="py-4 px-4">
                  ...
                  <tr>
                    <td colSpan={abaAtiva === 'pendentes' ? 10 : 9} className="py-8">
                      <EmptyState
                        icon={Users}
                        title="Nenhum eleitor encontrado"
                        description="Cadastre seu primeiro eleitor ou ajuste os filtros"
                        action={{ label: 'Adicionar eleitor', onClick: () => setNovoOpen(true) }}
                      />
                    </td>
                  </tr>
                ) : (
                  filtered.map(e => (
                    <tr key={e.id} className="border-b border-slate-50 hover:bg-blue-50/50 transition-colors cursor-pointer" onClick={() => setPreviewEleitor(previewEleitor?.id === e.id ? null : e)}>
                      ...
                      <td className="py-3 px-4">
                        <span className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-full">{getComunidadeNome(e.comunidade_id) || '—'}</span>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex flex-wrap gap-1">
                          {(e.tags || []).slice(0,2).map((t,i) => <span key={i} className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">{t}</span>)}
                          {(e.tags || []).length > 2 && <span className="text-[10px] text-slate-400">+{(e.tags || []).length - 2}</span>}
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${e.status === 'ativo' ? 'bg-green-50 text-green-600' : e.status === 'pendente' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-500'}`}>{e.status || '—'}</span>
                      </td>
                      <td className="py-3 px-4">
