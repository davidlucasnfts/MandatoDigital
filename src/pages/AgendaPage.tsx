...
import { Calendar, ChevronLeft, ChevronRight, Plus, Clock, MapPin, Pencil, Trash2 } from '@/lib/icons';
            ...
            eventos.slice(0, 5).map(e => (
              <div key={e.id} className="p-3 bg-slate-50 rounded-lg hover:bg-blue-50 transition-colors cursor-pointer group">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1"><h4 className="text-sm font-medium text-slate-800 truncate">{e.titulo}</h4><div className="flex items-center gap-2 mt-1 text-xs text-slate-500"><span className="flex items-center gap-1"><Clock className="w-3 h-3"/>{e.hora_inicio}</span><span className="flex items-center gap-1 truncate"><MapPin className="w-3 h-3"/>{e.local}</span></div><div className="text-xs text-slate-400 mt-1">{e.data}</div></div>
                  <div className="flex flex-col gap-1">
                    <button onClick={() => setEditEvento(e)} className="flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium bg-blue-50 text-blue-600 hover:bg-blue-100 rounded"><Pencil className="w-3 h-3"/>Editar</button>
                    <button onClick={() => { if (confirm('Excluir este evento?')) { remove(e.id); fetch(); } }} className="flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium bg-red-50 text-red-600 hover:bg-red-100 rounded"><Trash2 className="w-3 h-3"/>Excluir</button>
                  </div>
                </div>
              </div>
            ))}
