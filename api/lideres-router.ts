...
// Cliente postgres para queries raw (db.execute do Drizzle tem problemas com params)
let rawClient: ReturnType<typeof postgres> | null = null;
function getRawClient() {
  if (!rawClient) {
    rawClient = postgres(env.databaseUrl);
  }
  return rawClient;
}
...
export const lideresRouter = createRouter({
  ...
  produtividade: editorQuery.query(async ({ ctx }) => {
    try {
      ...
      // Usa cliente postgres direto para SQL raw (evita problemas do db.execute)
      ...
      const lideres = rows as unknown as Array<{
        id: string;
        nome: string;
