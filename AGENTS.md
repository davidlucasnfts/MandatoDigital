...
- **Responsividade automática** — ao criar/modificar qualquer página, dialog, tabela, grid ou lista, SEMPRE aplicar classes responsivas do Tailwind. Nunca esperar o usuário pedir — é obrigatório.
- **Padrões responsivos mínimos (mesmos da ComunicaçãoPageV2):**
  - Header: `flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3`
  - Stats: `grid grid-cols-2 lg:grid-cols-4 gap-2.5 lg:gap-4` (2 colunas mobile, 4 desktop)
  - Grids de cards: `grid sm:grid-cols-2 lg:grid-cols-3 gap-3` (1 coluna mobile, 2 tablet, 3 desktop)
  - Dialogs/modais: `w-full max-w-lg max-h-[80vh] overflow-y-auto` (largura total no mobile, limitada no desktop)
  - Botões lado a lado: manter em linha com gap, mas usar `flex-wrap` se necessário
  - Textos: `text-xl sm:text-2xl` para títulos, `text-xs sm:text-sm` para subtítulos
  - Ícones: `w-5 h-5 sm:w-6 sm:h-6` para adaptar tamanho
  - Espaçamento: `space-y-4 lg:space-y-6` para sections, `p-4 lg:p-6` para cards
...
| 040 | Evolution API v2.2.3 — `integration` incorreta causa "Invalid integration" | 19/06/2026 | SEMPRE usar `"integration": "EVOLUTION"` (não "WHATSAPP_BAILEYS", "BAILEYS", "WHATSAPP") ao criar instância na Evolution API v2.2.3. Verificar documentação da versão antes de assumir parâmetros |
| 041 | Expor PostgreSQL com `trust` no IP público | 19/06/2026 | NUNCA usar `trust` para IPs públicos no `pg_hba.conf`. Sempre `md5`/`scram-sha-256` + senha forte. Sempre restringir `listen_addresses = '127.0.0.1'` |
| 042 | Não documentar alteração de senha/infra no SESSION-CONTEXT.md | 19/06/2026 | SEMPRE que alterar senha, configuração de firewall, ou infraestrutura, documentar imediatamente no SESSION-CONTEXT.md → "Decisões Pendentes" com: o quê, onde, como fazer |
