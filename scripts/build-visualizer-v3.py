#!/usr/bin/env python3
"""
Gera visualizer.html com graph.json embutido - VERSAO 3 (corrigida)
Uso: python scripts/build-visualizer-v3.py
"""
...
# Converte para string JSON compacta (uma linha so)
graph_json_compact = json.dumps(graph_data, ensure_ascii=False, separators=(',', ':'))

# Parte 1: HTML ate o script
...
<div id="sidebar">
  ...
  <div id="info-panel">
    <h3>Detalhes</h3>
    <div id="info-content"><span class="empty">Clique em um no para ver detalhes</span></div>
  </div>
  <div id="godnodes">
...
<script>
// Dados embutidos do grafo
const GRAPH_DATA = '''

# Parte 2: JSON do grafo
html_part2 = ''';
...
nodesWithDegree.forEach(n => {
  ...
  if (!communities[c]) communities[c] = { color: (n.color && n.color.background) ? n.color.background : '#4E79A7', count: 0, label: 'Comunidade ' + c };
  communities[c].count++;
});
...
const nodesDS = new vis.DataSet(nodesWithDegree.map(n => ({
  id: n.id,
  label: n.label || n.id,
  title: (n.label || n.id) + '<br>Conexoes: ' + (n.degree || 0) + '<br>Comunidade: ' + (n.community || 0),
  ...
  if (params.nodes.length > 0) {
    ...
    if (node) {
      ...
      document.getElementById('info-content').innerHTML = 
        ...
        '<div class="field"><b>Conexoes:</b> ' + (node.degree || 0) + '</div>' +
        '<div class="field"><b>Comunidade:</b> ' + (node.community || 0) + '</div>' +
        '<div class="field"><b>Vizinhos:</b> ' + uniqueNeighbors.length + '</div>';
    }
  } else {
    document.getElementById('info-content').innerHTML = '<span class="empty">Clique em um no para ver detalhes</span>';
  }
});
...
# Junta tudo
html_content = html_part1 + graph_json_compact + html_part2

# Salva o arquivo
output_path = 'graphify-out/visualizer-v3.html'
