#!/usr/bin/env python3
"""
Versao limpa do visualizador Graphify com melhorias:
1. Fisica mais leve (nao trava)
2. God Nodes com tipo
3. Comunidades com nomes significativos
Base: graph.html original (que ja funciona perfeitamente)
"""
...
# 2. MELHORA 2: Adiciona botao Exportar PNG
...
# 3. MELHORA 3: Adiciona God Nodes na sidebar (antes do legend-wrap)
# Procura o ponto certo para inserir
godnodes_section = '''
...
# 4. MELHORA 4: Substitui "Community X" pelos nomes significativos na criacao da legenda
# O original cria a legenda com LEGEND.forEach - vamos modificar o label
community_map_json = json.dumps(community_names, ensure_ascii=False)

# Procura onde LEGEND e criado e adiciona o mapa de nomes
# Inserimos o mapa antes da criacao da legenda
legend_insertion = '''
...
# Encontra "const legendEl = document.getElementById('legend');" e insere antes
html = html.replace(
    "const legendEl = document.getElementById('legend');",
    legend_insertion + "const legendEl = document.getElementById('legend');"
)

# Modifica a criacao do label na legenda para usar o nome customizado
# Original: item.innerHTML = `<div class="legend-dot" style="background:${c.color}"></div><span class="legend-label">${c.label}</span>...
html = html.replace(
    '<span class="legend-label">${c.label}</span>',
    '<span class="legend-label">${COMMUNITY_NAMES[c.cid] || c.label}</span>'
)

# 5. MELHORA 5: Adiciona funcao getNodeType e God Nodes no final
js_final = '''

// Funcao para identificar tipo do no
...
# Salva
output_path = 'graphify-out/visualizer-enhanced.html'
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(html)

print('OK - Visualizador enhanced gerado:', output_path)
print('Melhorias:')
print('  - Fisica mais leve (60 iteracoes)')
print('  - Comunidades com nomes significativos')
print('  - Top 10 com tipo do arquivo')
print('  - Botao Exportar PNG')
